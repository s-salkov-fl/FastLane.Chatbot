...
public partial class GetLastMessages(IOptionsMonitor<Settings> settings)
{
	...
	public async Task<IReadOnlyList<ChatMessage>> InvokeActionAsync(IBrowser browser, string botNickName, ChatMember memberFilter, CancellationToken cancellationToken)
	{
		...
		string selector = (memberFilter != ChatMember.Bot) ? _settings.TikTokPageExpressions.ChatMessageContainerCommon
			 : string.Format(_settings.TikTokPageExpressions.ChatMessageContainerExactUserPattern, botNickName);
		...
		for (int i = elements.Length; i > 0; i--)
		{
			...
			IElementHandle userNameElement = await elements[i - 1].QuerySelectorAsync(_settings.TikTokPageExpressions.ChatMessageUserName)
